//! Tied-embedding `lm_head` derivation -- identical requirement and
//! algorithm to `loaders/huggingface::derived_lm_head` (that crate cannot
//! be depended on from here). A real GGUF export with `tie_word_embeddings:
//! true` genuinely omits `output.weight` entirely (llama.cpp's own GGUF
//! writer convention for tied checkpoints, matching Hugging Face's): a
//! synthetic `lm_head` tensor entry is added to the manifest, and a
//! payload-source wrapper answers a request for it by reading
//! `token_embedding`'s real bytes and transposing them, the same
//! row-major transpose `weight_layout.rs` implements for real, stored
//! projection weights.
...
/// vocab_size]`). Returns `true` if a synthetic tensor was appended.
...
/// source and transposing them; every other identity passes through
/// unchanged.
...
impl<S> DerivedLmHeadPayloadSource<S> {
    ...
    /// *not* the synthetic `lm_head` entry).
